...
load(file="/home/ruser/TextPrism/modelResult/model_#sentiment_10_0.999.RData")
...
for(i in seq(from=1, to=nrow(corpus), by=increment)){
  ...
  predResultDf$pos_score <- round(predResultDf$pred.pos * 100, digit=6)
  predResultDf$neg_score <- round(predResultDf$pred.neg * 100, digit=6)
  predResultDf <- subset(predResultDf, select = c(crawl_data_id, pos_score, neg_score))
  predResultDf$sentiment <- ifelse(predResultDf$pos_score >= predResultDf$neg_score,
                                   ifelse(predResultDf$pos_score == predResultDf$neg_score,"neu","pos"),"neg")
  #modelResult$user <- user
  print("End Prediction")
  print(summary(predResultDf))

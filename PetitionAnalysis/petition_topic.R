...
petitions = readRDS("./petitions.RDS")
petitions2 = readRDS("./petitions2.RDS")

petitions = rbind(petitions, petitions2)
rm(petitions2)

parsed = r_parser_r(petitions$content, useEn = T, language = "ko", korDicPath = "../FastCampus/dictionary.txt")

##################################################################
#Text Pre-processing
##################################################################
#Corpus 생성
corp=VCorpus(VectorSource(parsed))
...
#박스그래프로 분포 확인
boxplot(term_tfidf)
quantile(term_tfidf, probs = seq(0,1,0.1))
...
source("../FastCampus/Week_5/createJsonForChart_v2.R")
